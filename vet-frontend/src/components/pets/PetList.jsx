...
const PetList = () => {
  ...
  if (loading) return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-10 text-center text-sm text-gray-400 dark:text-gray-500">
      Φόρτωση...
    </div>
  );
  if (error) return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-10 text-center text-sm text-red-500 dark:text-red-400">
      {error}
    </div>
  );
    ...
    <>
      ...
      <div className="mb-3 relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-9 py-2.5 rounded-2xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-300 placeholder-gray-400 dark:placeholder-gray-500"
        />
        {query && (
          <button
            onClick={() => setQuery("")}
      ...
      <div className="overflow-x-auto bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
        {filtered.length === 0 ? (
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {filtered.map((pet) => (
                <tr key={pet._id} className="border-b border-gray-50 dark:border-gray-700/50 hover:bg-sky-50/20 dark:hover:bg-sky-900/10 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {pet.microchip
                      ? <span className="font-mono text-xs text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 border border-gray-100 dark:border-gray-600 px-2 py-0.5 rounded-lg">{pet.microchip}</span>
