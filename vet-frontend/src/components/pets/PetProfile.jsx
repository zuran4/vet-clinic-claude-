...
const PetProfile = ({ petId, initialTab }) => {
  ...
  if (loading) return (
    <div className="p-10 text-center text-sm text-gray-400 dark:text-gray-500">Φόρτωση...</div>
  );
  if (!pet) return (
    <div className="p-10 text-center text-sm text-red-500 dark:text-red-400">Δεν βρέθηκε κατοικίδιο.</div>
  );
      ...
      <div className="bg-white dark:bg-win-elevated/50 rounded-2xl border border-gray-100 dark:border-win-border-light shadow-sm px-4 py-4 flex items-center gap-4">
        ...
        {pet.registrySnapshot && (
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-medium px-3 py-2 rounded-xl bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 text-indigo-600 dark:text-indigo-300 transition-colors flex-shrink-0"
          >
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 truncate">{pet.owner.name}</p>
                    {pet.owner.phone && (
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-xl bg-amber-50 dark:bg-amber-900/20 hover:bg-amber-100 dark:hover:bg-amber-900/30 text-amber-700 dark:text-amber-400 transition-colors flex-shrink-0"
                >
                  <RefreshCcw className="w-3.5 h-3.5" /> Αλλαγή
                </button>
              </div>
            ) : (
              <p className="text-sm text-gray-400 dark:text-gray-500">Δεν έχει οριστεί ιδιοκτήτης</p>
            )}
            {ownerChanged && (
              <button
