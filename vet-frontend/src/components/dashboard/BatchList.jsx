...
const BatchList = ({ productId, isOpen, onQuantity, summaryQty, category }) => {
  ...
  const getBatchStatus = (expirationDate) => {
    if (!expirationDate) return { label: "—", className: "bg-gray-100 text-gray-500" };
    const diff = dayjs(expirationDate).diff(dayjs(), "day");
    if (diff < 0)   return { label: "Έληξε",              className: "bg-red-100 text-red-700" };
    if (diff <= 30) return { label: `Λήγει σε ${diff}μ`,  className: "bg-amber-100 text-amber-700" };
    if (diff <= 60) return { label: `Λήγει σε ${diff}μ`,  className: "bg-yellow-100 text-yellow-700" };
    return           { label: "Εντάξει",                  className: "bg-green-100 text-green-700" };
  };
  ...
  const getRowBg = (expirationDate) => {
    ...
    if (diff <= 60) return "bg-amber-50/40";
    return "";
  };
    ...
    <div className="rounded-2xl overflow-hidden border border-orange-100 shadow-sm mt-1">
      ...
      <div className="bg-gradient-to-r from-orange-50 to-amber-50 border-b border-orange-100 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-orange-700">Παρτίδες Προϊόντος</span>
        </div>
        <div className="flex items-center gap-2">
      ...
      {loading ? (
        ...
        <div className="bg-white px-4 py-6 text-center text-sm text-gray-400">Δεν υπάρχουν παρτίδες.</div>
      ) : (
        <div className="bg-white overflow-x-auto max-h-72 overflow-y-auto">
          <table className="w-full text-sm">
            <thead className="sticky top-0 z-10">
              <tr className="bg-gray-50 border-b border-gray-100">
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Παρτίδα</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Κατάσταση</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Ποσότητα</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Τιμολόγιο</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Ημ. Αγοράς</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Ημ. Λήξης</th>
              </tr>
            </thead>
            ...
            <tbody className="divide-y divide-gray-50">
              {batches.map((batch, idx) => {
                ...
                return (
                  <tr key={batch._id || idx} className={`${rowBg} hover:bg-orange-50/30 transition-colors`}>
                    ...
                    <td className="px-4 py-2.5 font-semibold text-gray-700">{batch.quantity}</td>
                    <td className="px-4 py-2.5 text-xs text-gray-400 font-mono">{batch.invoiceNumber || "—"}</td>
            ...
            <tfoot className="sticky bottom-0 bg-orange-50 border-t border-orange-100">
              <tr>
                <td colSpan={2} className="px-4 py-2.5 text-right text-xs font-semibold text-orange-600 uppercase tracking-wide">
                  Σύνολο
                </td>
