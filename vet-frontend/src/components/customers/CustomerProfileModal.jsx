  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className={`text-sm font-semibold leading-snug ${isEmpty ? "text-gray-300" : "text-gray-800"}`}>
          {value}
        </p>
      </div>
    </div>
...
const CustomerProfileModal = ({ customer, onClose, onEdit, onPurchases }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div
        className="relative w-full max-w-[600px] rounded-2xl overflow-hidden shadow-2xl z-10 max-h-[90vh] flex flex-col bg-white"
      ...
      >
        ...
        {/* Content */}
        <div className="bg-gray-50 overflow-y-auto flex-1 p-5 space-y-4">
          ...
          <div className="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-50">
              ...
              <div>
                ...
                {city && <p className="text-xs text-gray-400">{display(city)}</p>}
              </div>
            </div>
          ...
          {notes && (
            <div className="bg-white rounded-2xl border border-gray-100 p-4">
              <h3 className="text-sm font-semibold text-gray-700 mb-1 flex items-center gap-2">
                <StickyNote className="w-4 h-4 text-amber-500" />Σημειώσεις
              </h3>
          ...
          {notifications && (
            <div className="bg-white rounded-2xl border border-gray-100 p-4">
              <h3 className="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                <Bell className="w-4 h-4 text-indigo-400" />Ειδοποιήσεις
              </h3>
                ...
                {NOTIFICATION_LABELS.map(({ key, label }) => (
                  <span
                    ...
                    className={`text-xs px-2.5 py-1 rounded-full font-medium ${
                      notifications[key] ? "bg-emerald-50 text-emerald-600" : "bg-gray-100 text-gray-400"
                    }`}
                  >
          ...
          {/* Κατοικίδια */}
          <div className="bg-white rounded-2xl border border-gray-100 p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-1">Κατοικίδια</h3>
            <CustomerPetsExpanded ownerId={_id} />
          </div>
        </div>

        {/* Footer */}
        <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-gray-100 bg-white flex-shrink-0">
