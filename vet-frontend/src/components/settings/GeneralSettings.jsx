...
const INPUT = "mt-1 w-full border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 shadow-sm focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100";
...
const GeneralSettings = ({ settings, setSettings }) => {
  ...
  return (
    <div className="space-y-6 p-6 bg-white dark:bg-gray-800 rounded-2xl shadow">
      ...
      {/* Προσωπικό */}
      <div className="rounded-2xl border border-gray-200 dark:border-gray-600 overflow-hidden">
        <div className="bg-gradient-to-r from-indigo-50 to-violet-50 dark:from-indigo-900/30 dark:to-violet-900/20 border-b border-gray-200 dark:border-gray-600 px-5 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {localSettings.staff.map((member, i) => (
              <div key={i} className="flex items-center gap-4 px-5 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                ...
                <span className={`text-xs font-semibold px-3 py-1 rounded-full flex-shrink-0 ${ROLE_STYLE[member.role] || "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300"}`}>
                  {member.role}
                </span>
                <button
                  type="button"
