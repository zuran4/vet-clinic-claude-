      ...
      <div className="space-y-4">
        ...
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-amber-800">Βήμα 1: Κατέβασε το template</p>
        ...
        <div className="flex items-start gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500">Έγκυρες κατηγορίες:</p>
            <div className="flex flex-wrap gap-1 mt-1">
              {VALID_CATEGORIES.map((c) => (
                <span key={c} className="bg-orange-100 text-orange-700 text-xs px-2 py-0.5 rounded-full font-medium">
                  {c}
                </span>
              ))}
            </div>
            <p className="text-xs text-gray-400 mt-1">Οτιδήποτε άλλο → "Άλλο" αυτόματα.</p>
          </div>
        </div>
        ...
        {!result && (
          <div>
            ...
            <button
              ...
              className="w-full border-2 border-dashed border-gray-200 hover:border-orange-300 rounded-xl px-4 py-6 flex flex-col items-center gap-2 text-gray-400 hover:text-orange-500 transition-colors"
            >
            ...
            <div className="overflow-x-auto rounded-xl border border-gray-100">
              <table className="min-w-full text-xs">
                <thead className="bg-gray-50">
                  <tr>
                    {PREVIEW_COLS.map((c) => (
                      <th key={c} className="px-3 py-2 text-left text-gray-500 font-semibold whitespace-nowrap">
                        {FIELD_LABELS[c]}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody className="divide-y divide-gray-50">
                  {rows.slice(0, 5).map((row, i) => (
                    <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}>
                      {PREVIEW_COLS.map((c) => (
                        <td key={c} className="px-3 py-2 text-gray-700 max-w-[100px] truncate">
        ...
        {result && (
          <div className="space-y-3">
            ...
            {result.errors?.length > 0 && (
              <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 space-y-1 max-h-40 overflow-y-auto">
                <p className="text-xs font-semibold text-red-700 sticky top-0 bg-red-50 pb-1">
