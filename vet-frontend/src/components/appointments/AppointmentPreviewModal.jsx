...
import DocumentScanEditor from "./DocumentScanEditor.jsx";
...
function AlertCard({ pet, customer, onPetSaved, onCustomerSaved }) {
  const [customerAlert, setCustomerAlert] = useState(customer?.alert || "");
  const [petAlert, setPetAlert] = useState(pet?.alert || "");
  const [savingCustomer, setSavingCustomer] = useState(false);
  const [savingPet, setSavingPet] = useState(false);
  const [error, setError] = useState("");

  useEffect(() => {
    setCustomerAlert(customer?.alert || "");
    setPetAlert(pet?.alert || "");
    setError("");
  }, [pet?._id, customer?._id]);

  if (!pet && !customer) {
    return <PlaceholderCard icon={AlertCircle} title="Alert" color="amber" />;
  }
  ...
  const handleSaveCustomer = async () => {
    ...
    setError("");
    try {
      const updated = await updateCustomer(customer._id, { alert: customerAlert.trim() });
      toast.success("✅ Η προειδοποίηση πελάτη αποθηκεύτηκε.");
  ...
  const handleSavePet = async () => {
    ...
    setError("");
    try {
      const updated = await updatePet(pet._id, { alert: petAlert.trim() });
      toast.success("✅ Η προειδοποίηση κατοικιδίου αποθηκεύτηκε.");
      onPetSaved?.(updated);
    } catch (err) {
      setError(err.message || "Αποτυχία αποθήκευσης.");
    } finally {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col items-center text-center py-1">
        <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-3 bg-amber-50 dark:bg-amber-900/20 text-amber-500 dark:text-amber-400">
          <AlertCircle className="w-5 h-5" />
        </div>
        <p className="text-sm font-semibold text-gray-700 dark:text-gray-200">Μόνιμες προειδοποιήσεις</p>
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
          Παραμένουν ορατές σε κάθε επόμενο ραντεβού
        </p>
      </div>
      ...
      {customer && (
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-gray-500 dark:text-gray-400">
            Για τον πελάτη ({customer.name})
          </label>
          <textarea
            value={customerAlert}
            onChange={(e) => setCustomerAlert(e.target.value)}
            placeholder="π.χ. Ο πελάτης είναι περίεργος..."
            rows={2}
          ...
          <button
            ...
            disabled={savingCustomer || customerAlert.trim() === (customer.alert || "")}
            className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-white text-xs font-semibold transition-colors shadow-sm"
          >
            {savingCustomer ? "Αποθήκευση..." : "Αποθήκευση για πελάτη"}
          </button>
        </div>
      )}
      ...
      {pet && (
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-gray-500 dark:text-gray-400">
            Για το κατοικίδιο ({pet.name})
          </label>
          <textarea
            value={petAlert}
            onChange={(e) => setPetAlert(e.target.value)}
            placeholder="π.χ. Δαγκώνει, χρειάζεται φίμωτρο..."
            rows={2}
            className={`${inputClass} resize-none`}
          />
          <button
            ...
            disabled={savingPet || petAlert.trim() === (pet.alert || "")}
            className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-white text-xs font-semibold transition-colors shadow-sm"
          >
            {savingPet ? "Αποθήκευση..." : "Αποθήκευση για κατοικίδιο"}
          </button>
        </div>
      )}
...
function PetFileCard({ pet, onSaved }) {
  ...
  useEffect(() => { setError(""); setScanFile(null); setShowLiveCamera(false); }, [pet?._id]);

  if (!pet) {
    return <PlaceholderCard icon={StickyNote} title="Αρχείο" color="slate" />;
  }
  ...
  const handleCameraClick = () => {
    // Ζωντανή σάρωση μέσα στη σελίδα όπου υποστηρίζεται (χρειάζεται HTTPS)·
    // αλλιώς fallback στη native κάμερα της συσκευής.
  ...
  const doUpload = async (file) => {
    ...
    setUploading(true);
    try {
      const updated = await uploadPetFile(pet._id, file);
      toast.success("✅ Το αρχείο ανέβηκε.");
      onSaved?.(updated);
    } catch (err) {
      setError(err.message || "Αποτυχία ανεβάσματος αρχείου.");
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleFileChange = (e) => {
    ...
    if (!file) return;
    setError("");
    // Έγγραφα/φωτογραφίες περνούν πρώτα από τον scanner για αυτόματη περικοπή·
    // PDF/Word/Excel ανεβαίνουν κατευθείαν.
  ...
  if (showLiveCamera) {
    return (
      <LiveCameraScanner
        ...
        onFallback={() => { setShowLiveCamera(false); cameraInputRef.current?.click(); }}
        onCapture={(file) => { setShowLiveCamera(false); setScanFile(file); }}
      />
    );
  }
  ...
  const handleDelete = async (fileId) => {
    ...
    setError("");
    try {
      const result = await deletePetFile(pet._id, fileId);
      onSaved?.({ ...pet, files: result.files });
      toast.success("🗑️ Το αρχείο διαγράφηκε.");
    } catch (err) {
      setError(err.message || "Αποτυχία διαγραφής αρχείου.");
    } finally {
      setDeletingId(null);
    }
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-col items-center text-center py-2">
        <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-3 bg-slate-100 dark:bg-slate-800/40 text-slate-500 dark:text-slate-400">
          <StickyNote className="w-5 h-5" />
        </div>
        <p className="text-sm font-semibold text-gray-700 dark:text-gray-200">Αρχεία</p>
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{pet.name}</p>
      </div>
      ...
      {files.length > 0 && (
        <div className="space-y-1.5 max-h-56 overflow-y-auto">
          {files.map((f) => (
            <div
              key={f._id}
              className="flex items-center gap-2 px-3 py-2 rounded-xl border border-gray-100 dark:border-win-border-light bg-white dark:bg-win-elevated/50"
            >
              ...
              <a
                ...
                className="flex-1 min-w-0 text-xs text-gray-700 dark:text-gray-200 truncate hover:text-indigo-600 dark:hover:text-indigo-400"
                title={f.name}
              >
                {f.name}
              </a>
              <button
                ...
                className="w-6 h-6 rounded-lg flex items-center justify-center text-gray-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors flex-shrink-0 disabled:opacity-50"
              >
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          disabled={uploading}
          className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-slate-500 hover:bg-slate-600 disabled:opacity-50 text-white text-sm font-semibold transition-colors shadow-sm"
        >
          <Upload className="w-4 h-4" />
          {uploading ? "Ανέβασμα..." : "Από αρχεία"}
        </button>
        <button
          type="button"
              ...
              >
                ...
                <div className="p-4 overflow-y-auto">
                  {quickCard === "chip" ? (
                    ...
                    <PetFileCard
                      pet={fullPet}
                      onSaved={(updated) => setFullPet(updated)}
                    />
                  ) : quickCard === "alert" ? (
                    <AlertCard
                      pet={fullPet}
                      customer={fullCustomer}
                      onPetSaved={(updated) => setFullPet(updated)}
                      onCustomerSaved={(updated) => setFullCustomer(updated)}
                    />
                  ) : quickCard !== "history" ? (
